"""
Simulate an RL framework to optimize a manufacturing process, where actions represent different machine settings and rewards are based on product quality. Implement the environment, policy, and value function in Python.
"""
...
def exp18():
    n_settings = 5
    quality_mean = np.array([0.5, 0.6, 0.7, 0.65, 0.4])
    Q = np.zeros(n_settings)
    counts = np.zeros(n_settings)
    for t in range(1000):
        a = epsilon_greedy(Q, 0.1)
        r = np.random.normal(quality_mean[a], 0.1)
        counts[a] += 1
        Q[a] += (r - Q[a]) / counts[a]
    print("exp18", Q)
